...
public class DowblodeListDialog extends BaseDialogFragment
{
    public static final String TAG = "DownlodeListDialog";
    DownlodeLIstDialogListener mListener;
    DownlodelistDialogBinding binding;
    Dialog dialog;
    DowblodeListDialog _self;
    ProjectDialogListAdapter projectDialogListAdapter;
    List<String> projectList = new ArrayList<>();
    public static DowblodeListDialog getInstance(DowblodeListDialog.DownlodeLIstDialogListener listener)
    {
        DowblodeListDialog frag = new DowblodeListDialog();
    ...
    {
        binding = DataBindingUtil.inflate(inflater, R.layout.downlodelist_dialog, container, false);
    ...
    public void setupListener(DownlodeLIstDialogListener listener)
    {
        mListener = listener;
    }

    private DownlodeLIstDialogListener getListener()
    {
        DownlodeLIstDialogListener listener = mListener;

        if (listener==null && getTargetFragment()!=null && getTargetFragment() instanceof DownlodeLIstDialogListener)
            listener = (DownlodeLIstDialogListener) getTargetFragment();

        if (listener==null && getActivity()!=null && getActivity() instanceof DownlodeLIstDialogListener)
            listener = (DownlodeLIstDialogListener) getActivity();

        return listener;
    }

    public interface DownlodeLIstDialogListener {
        void onOk(DowblodeListDialog dialogFragment);
        default void onCancel(DowblodeListDialog dialogFragment){}
    }
